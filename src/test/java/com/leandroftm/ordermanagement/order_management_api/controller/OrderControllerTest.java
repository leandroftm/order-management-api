...
public class OrderControllerTest {
    ...
    @Test
    @WithMockUser(roles = "ADMIN")
    void shouldReturnAllOrdersByStatusSuccessfully() throws Exception {
        Page<OrderResponse> page = createPage();

        when(orderService.getAllOrders(any(Status.class), any(Pageable.class))).thenReturn(page);
    ...
    void shouldCreateOrderSuccessfully() throws Exception {
        ...
        User user = new User("test@test.com", "123456", Role.USER);
        ReflectionTestUtils.setField(user, "id", 1L);
        UserPrincipal userPrincipal = new UserPrincipal(user);

        when(orderService.createOrder(1L, request)).thenReturn(1L);
    ...
    void shouldCancelOrderSuccessfully() throws Exception {
        mockMvc.perform(post("/orders/{id}/cancel", 1L))
                .andExpect(status().isNoContent());
    }
